...
class AMRBartTokenizer(BartTokenizer):
    ...
    def init_amr_vocabulary(self, pred_min=5):
        ...
        tokens = []
        for line in Path('models/spring_amr/data/vocab/predicates.txt').read_text().strip().splitlines():
            tok, count = line.split()
            if int(count) >= pred_min:
                tokens.append(tok)
                
        for tok in Path('models/spring_amr/data/vocab/additions.txt').read_text().strip().splitlines():
            tokens.append(tok)

        for tok in Path('models/spring_amr/data/vocab/recategorizations.txt').read_text().strip().splitlines():
    ...
    def batch_encode_sentences(self, sentences, device=torch.device('cpu')):
        ...
        # updated from pad_to_max_length=True
